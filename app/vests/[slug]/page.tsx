...
export async function generateStaticParams() {
  const slugs = await getAllVestSlugs();
...
// ── Page ─────────────────────────────────────────────────────────
export default async function VestReviewPage({ params }: PageProps) {
  const { slug } = await params;
  const [r, stats] = await Promise.all([
    getVestReview(slug),
    getCategoryStats('vests'),
  ]);
  ...
  return (
    <div className="bg-sand font-sans text-carbon">
      ...
      <section className="border-b border-rule px-8 py-5">
        <div className="mx-auto max-w-7xl">
          <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-rust">spec sheet</span>
          <div className="mt-3 grid grid-cols-1 gap-x-8 gap-y-3 sm:grid-cols-2 lg:grid-cols-4">
            <SpecGroup label="capacity" items={[
              ['capacity', r.capacity_l != null ? `${r.capacity_l}L` : null],
              ['weight', r.in_house_weight_g ? `${r.in_house_weight_g}g · in-house` : r.weight_g != null ? `${r.weight_g}g` : null],
            ]} />
            <SpecGroup label="pockets" items={[
              ['front', r.front_pockets != null ? String(r.front_pockets) : null],
              ['back', r.back_pockets != null ? String(r.back_pockets) : null],
            ]} />
            <SpecGroup label="features" items={[
              ['flasks', r.soft_flask_included ? 'included' : 'not included'],
              ['UTMB', r.utmb_compliant ? 'compliant' : '—'],
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-0 border-b border-rule lg:grid-cols-[1.55fr_1fr]">
        <article className="border-b border-rule px-4 pb-10 pt-6 sm:px-6 lg:border-b-0 lg:border-r lg:px-8">
          ...
          {r.image_url && (
            <figure className="mt-6">
              <div className="relative aspect-[16/9] overflow-hidden bg-sand-deep">
                <Image src={r.image_url} alt={`${r.brand} ${r.model}`} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
              </div>
              <figcaption className="mt-2 font-mono text-[10.5px] uppercase tracking-[0.12em] text-ink-50">
                Fig. 01 · {r.brand} {r.model}
              </figcaption>
            </figure>
          )}
          ...
          <AiTransparency tester={r.tester} editor={r.human_editor} milesTested={r.miles_tested} weeksTested={r.weeks_tested} testTerrain={r.test_terrain} aiDraftedAt={r.ai_drafted_at} humanEditedAt={r.human_edited_at} reviewId={r.id} />
        </article>
        ...
        <aside className="px-4 pb-10 pt-6 sm:px-6 lg:px-8">
          ...
          <div className="mt-3.5 rounded border border-rule bg-paper p-[18px]">
            ...
            <div className="mt-2 font-mono text-[10px] text-ink-50">vs {stats.count} vests in category</div>
          </div>
