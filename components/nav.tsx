...
export default async function Nav() {
  ...
  return (
    <nav className="sticky top-0 z-30 bg-sand">
      {/* Topbar */}
      <div className="bg-carbon px-4 py-2 font-mono text-[10.5px] tracking-[0.06em] text-sand sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between overflow-x-auto">
          <div className="flex gap-4 whitespace-nowrap">
            <span><span className="text-ochre">●</span> RGD/INDEX</span>
            <span>SKU·{counts.total}</span>
            <span className="hidden sm:inline">SHOES·{counts.shoes}</span>
            <span className="hidden sm:inline">VESTS·{counts.vests}</span>
      ...
      {/* Main row */}
      <div className="relative border-b border-rule px-4 py-3.5 sm:px-6 lg:px-8">
